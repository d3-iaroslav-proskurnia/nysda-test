...
export class AddNewClientPage extends GlobalActionsAndElements {
    ...
    async createRandomNewClientViaApiAndReturnData(request: APIRequestContext){
        ...
        const createClientResponse = await request.post(`${process.env.API_URL}clients?api-version=1.0`,
            {
                headers: {
        ...
        const getFullClientDataResponse = await request.get(`${process.env.API_URL}clients/${clientId}?api-version=1.0`,
            {
                headers: {
                    'content-type': 'application/json',
                    authorization: `Bearer ${process.env.ADMIN_ACCESS_TOKEN}`,
