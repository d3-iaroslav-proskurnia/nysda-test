...
export class AddNewCasePage extends GlobalActionsAndElements {
    ...
    async createNewFamilyCaseViaApiAndReturnFullData(request: APIRequestContext, clientNameId:string){
        ...
        const createCaseResponse = await request.post(`${process.env.API_URL}cases?api-version=1.0`,
            {
                headers: {
        ...
        const getFullCaseDataResponse = await request.get(`${process.env.API_URL}cases/${caseId}?api-version=1.0`,
            {
                headers: {
                    'content-type': 'application/json',
                    authorization: `Bearer ${process.env.ADMIN_ACCESS_TOKEN}`,
