...
export class GlobalActionsAndElements {
  ...
  constructor(page: Page) {
    ...
    this.anyInnerDialogModal = this.page.locator(
      '[role="dialog"][data-popper-placement]',
    );
    ...
    // page stepper
    this.pageStepper = this.page.locator(
  ...
  public convertCaseFileNumberToFormattedString(
    caseFileNumberString: string,
  ): string {
    /*** Convert fileNumber string from "2551037" to "25-51037" ***/
    return (
      caseFileNumberString.slice(0, 2) + '-' + caseFileNumberString.slice(2)
    );
  }
  ...
  async fillComplexDateFieldWithFormattedDate(
    groupName: string,
    ...
    await this.getComplexFieldContainerLocatorByGroupName(groupName)
      .getByLabel('Month')
